...
static void rtnl_addr(int iface, char *caddr, uint32_t prefix, int type)
{
	union {
		in_addr_t ip;
		struct in6_addr ip6;
	...
	if (!inet_pton(AF_INET, caddr, &addr)) {
		if (!inet_pton(AF_INET6, caddr, &addr)) {
			perror("inet_pton");
			exit(EXIT_FAILURE);
		}
		family = AF_INET6;
	...
	nlh->nlmsg_type	= type; // RTM_NEWADDR or RTM_DELADDR
	...
	//ifm->ifa_flags = IFA_F_PERMANENT | IFA_F_NODAD;
...
int main(int argc, char *argv[])
{
	...
	rtnl_addr(iface, argv[2], prefix, RTM_NEWADDR);
	rtnl_addr(iface, argv[2], prefix, RTM_DELADDR);
